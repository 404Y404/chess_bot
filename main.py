...
class Board():
    ...
    def parse(self, old_hod):
        end = self.browser.find_element(By.XPATH, "/html/body/div[2]/div[2]/chess-board/div[contains(@class, 'highlight')][1]")
        start = self.browser.find_element(By.XPATH, "/html/body/div[2]/div[2]/chess-board/div[contains(@class, 'highlight')][2]")
        hod = to_bot(start.get_attribute("class")) + to_bot(end.get_attribute("class"))
        while hod == old_hod or hod[2:] + hod[:2] == old_hod:
            end = self.browser.find_element(By.XPATH, "/html/body/div[2]/div[2]/chess-board/div[2]")
            start = self.browser.find_element(By.XPATH, "/html/body/div[2]/div[2]/chess-board/div[3]")
            hod = to_bot(start.get_attribute("class")) + to_bot(end.get_attribute("class"))
    ...
    def move(self, element, x, y, is_end, figura):
